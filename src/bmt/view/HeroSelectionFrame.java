...
import bmt.model.Archer;
import bmt.model.Hero;
import bmt.model.Mage;
import bmt.model.Warrior;
...
public class HeroSelectionFrame extends JFrame {
    ...
    private void scaleFonts() {
        ...
        float scale = Math.max(0.8f, Math.min(2.5f, w / 490f));

        if (titleLabel != null)
            titleLabel.setFont(new Font("SansSerif", Font.BOLD, Math.round(22 * scale)));
        if (nameLabel != null)
            nameLabel.setFont(new Font("SansSerif", Font.PLAIN, Math.round(14 * scale)));
        if (classLabel != null)
            classLabel.setFont(new Font("SansSerif", Font.PLAIN, Math.round(14 * scale)));
        if (infoLabel != null)
            infoLabel.setFont(new Font("SansSerif", Font.ITALIC, Math.round(12 * scale)));
        if (startBtn != null)
            startBtn.setFont(new Font("SansSerif", Font.BOLD, Math.round(15 * scale)));
        if (nameField != null)
            nameField.setFont(new Font("SansSerif", Font.PLAIN, Math.round(14 * scale)));
        if (classCombo != null)
            classCombo.setFont(new Font("SansSerif", Font.PLAIN, Math.round(13 * scale)));
    ...
    private JPanel buildPanel() {
        ...
        String[] classes = {
                "Warrior  (HP:150 — tanky bruiser)",
                "Mage     (HP:90  — high-damage spells)",
                "Archer   (HP:110 — swift & precise)"
        };
        ...
        infoLabel = new JLabel("You will face an AI opponent of the same class.", SwingConstants.CENTER);
        ...
        startBtn.addActionListener(e -> {
            ...
            Hero opponent = createHero(idx, "Shadow " + player.getHeroClass());
